...
class MainActivity2 : AppCompatActivity(), SensorEventListener {
    ...
    override fun onSensorChanged(event: SensorEvent) {
       ...
       */
        ...
        var cal = event.values[0] + event.values[1] + event.values[2]
        addToWindow(cal)
        ...
        if (Input_num?.getText().toString() == ""&& linear_acceleration[1] > thresHold!! && cur_time - pre_time > 2000){
            Toast.makeText(applicationContext,"please type a number", Toast.LENGTH_SHORT).show()
            pre_time = cur_time
        } else {
    ...
    fun addToWindow(x: Float) { // Shift everything one to the left
        for (i in 1 until slidingWindow.size) {
            slidingWindow[i - 1] = slidingWindow[i]
        }
        // Add the new data point
        slidingWindow[slidingWindow.size - 1] = x.toDouble()
    }
